...
NVIDIA_INC_DIRS = []
NVCC = 'nvcc'
for path in ('/usr/local/cuda', '/opt/cuda','/usr/local/cuda','/usr/lib/nvidia-cuda-toolkit','/opt/cuda'):
...
class GPUBuilder(build_ext):
    ...
    def run(self):
        if '_cpyrit_cudakernel.ptx.h' in os.listdir('./'):
            ...
            nvcc_cmd = NVCC + bit_flag + ' -Xcompiler "-fPIC" --ptx ./_cpyrit_cudakernel.cu'
